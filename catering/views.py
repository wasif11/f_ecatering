...
def gigPricing(request):
    if request.method == "POST":
        ...
        print(request.POST.get(
            'packagerevisionsp'))
        request.session['packagepriceb'] = request.POST.get(
            'packagepriceb')
...
def gigGallery(request):
    ...
    if request.method == "POST":
        ...
        gig_package_detail_standard_price='', gig_package_detail_premium_price='',  gig_package_details='', gig1_image=imageform.cleaned_data['gig1_image'], gig2_image=imageform.cleaned_data['gig2_image'], gig3_image=imageform.cleaned_data['gig3_image'], gig_user=request.user.username)
        gig.save()
        return redirect('home')
    else:
       form = ImageForm()
...
def searchPage(request):
       ...
       my_session_var_value = request.session.get('search_query')
       print(my_session_var_value)
